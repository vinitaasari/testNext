...
const TrendingSkillDetail = (props) => {
  ...
  const { skillId } = props.match.params;
  ...
  const getCoursesOfSkill = async (apiBody) => {
    try {
  ...
  return (
    <AppWrapper>
      <SEO
        title={`Midigiworld - ${history.location.state.name || ""} Courses`}
      ...
      <Container maxWidth="lg" classes={{ root: classes.container }}>
        <Typography classes={{ root: classes.heading }}>
          All {history.location.state.name || null} Courses
        </Typography>
